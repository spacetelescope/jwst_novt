...
using the Python tools, see the `software documentation <novt>`__ or the
example notebooks in the
`source distribution <https://github.com/spacetelescope/novt/novt/notebooks>`__. The below
sections describe usage for the web application.
...
`Science Instrument Aperture File (SIAF) <https://jwst-docs.stsci.edu/jwst-observatory-characteristics/jwst-observatory-coordinate-system-and-field-of-regard/jwst-instrument-ideal-coordinate-systems>`
...
`LEGUS Public Access Table <https://archive.stsci.edu/prepds/legus/dataproducts-public.html>`
...
once: the MSA quadrants footprints, the fixed slits, and the IFU footprint on the
...
A customized version of this tool is provided in NOVT; see :ref:`show_timeline`.
...
In order to display a NIRCam dither pattern, simply select the pattern by name and
...
with the MSA footprint over plotted on M51.
...
.. figure:: images/nirspec_with_dither.png
   ...
   The 8NIRSPEC FULLBOX dither pattern shown in blue together with the NIRSpec
   apertures in red. Note that the dither pattern is designed to cover the MSA
   footprint at any aperture position angle. The yellow labels are not part of the
   application, they are used here to show the position of the MSA quadrants,
...
available aperture position angles. Both are included in the plot for NIRCam
and NIRSpec. and the V2V3 plane. Zoom in on the plot to see the details.
...
`NOVT GitHub repository <https://github.com/spacetelescope/novt>`__.

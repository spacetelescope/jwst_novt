...
class TestDisplay(object):
    ...
    def test_bqplot_catalog_errors(self, loaded_imviz, tmp_path,
                                   catalog_file_2col):
        fig = loaded_imviz.default_viewer.figure
        wcs = loaded_imviz.default_viewer.state.reference_data.coords
    ...
    def test_average_pa(self, timeline_data, inst, mean_value, mode_value):
        ...
        label = u._average_pa(
            timeline_data['Time'], timeline_data[f'{inst}_min_PA'],
            timeline_data[f'{inst}_max_PA'], method='mode')
        ...
        label = u._average_pa(
            timeline_data['Time'], timeline_data[f'{inst}_min_PA'],
            timeline_data[f'{inst}_max_PA'], method='mode',
            min_time=Time('2022-01-04'), max_time=Time('2022-01-05'))
    ...
    def test_bqplot_timeline_colors(self, inst, v3pa, n_plot):
        ...
        fig = bqplot.Figure()
        colors = ['red', 'blue']
        u.bqplot_timeline(fig, ra, dec, instrument=inst, show_v3pa=v3pa,
                          colors=colors)
    ...
    def test_bqplot_timeline_no_data(self, mocker, inst):
        ...
        u.bqplot_timeline(fig, ra, dec, start_date='2020-01-01',
                          end_date='2020-02-01', instrument=inst)
        assert len(fig.marks) == 1
    ...
    def test_bqplot_timeline_label_callback(self):
        ...
        dec = 47.1951868
        fig = bqplot.Figure()
        u.bqplot_timeline(fig, ra, dec, instrument='NIRSpec', show_v3pa=False,
                          start_date='2022-01-05', end_date='2022-01-09')
